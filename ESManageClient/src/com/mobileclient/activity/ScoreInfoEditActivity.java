...
public class ScoreInfoEditActivity extends Activity {
		...
		btnUpdate.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					...
					intent.setClass(ScoreInfoEditActivity.this, ScoreInfoListActivity.class);
					startActivity(intent); 
					ScoreInfoEditActivity.this.finish();
				} catch (Exception e) {}
			}
		});
	}
	...
	private void initViewData() {
		...
		for (int i = 0; i < student_QQ287307421List.size(); i++) {
			if (scoreInfo.getStudentNumber().equals(student_QQ287307421List.get(i).getStudentNumber())) {
				this.spinner_studentNumber.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < courseInfo_QQ254540457List.size(); i++) {
			if (scoreInfo.getCourseNumber().equals(courseInfo_QQ254540457List.get(i).getCourseNumber())) {
